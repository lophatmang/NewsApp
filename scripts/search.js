...
const maxPageFun = async function (page, pageSize, keySearch) {
  const data = await userLogin.search(page, pageSize, keySearch);
...
const renderNews = async function (page, pageSize, keySearch) {
...
btnSubmit.addEventListener("click", function () {
  if (!inputQuery.value) {
    swal(
      ...
      "Hãy điền từ khóa bạn muốn tìm kiếm",
      "error"
    ); // có thể dùng "warning", "error", "success" and "info"
  } else {
    keySearch = inputQuery.value;
    maxPageFun(numberPage, currentUser.pageSize, keySearch);
    renderNews(numberPage, currentUser.pageSize, keySearch);
    content.querySelector("nav").classList.remove("hide");
  }
});
